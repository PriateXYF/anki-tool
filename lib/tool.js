...
// 所有可处理的类型
const all_types = ['cloze', 'question', 'word', 'wordlist']
        ...
        switch (item.type) {
            case 'cloze':
                item.content.forEach(mdstr => {
                    const cloze_list = parse.parseMdCloze(marked.parse(mdstr))
                    res.content.push(...cloze_list)
                })
                break;
            case 'question':
                item.content.forEach(mdstr => {
                    const question_list = parse.parseMdQuestion(marked.parse(mdstr))
                    res.content.push(...question_list)
                })
                break;
            case 'word':
                item.content.forEach(mdstr => {
                    const word_list = parse.parseMdWord(marked.parse(mdstr))
                    res.content.push(...word_list)
                })
                break;
            case 'wordlist':
                item.content.forEach(mdstr => {
                    const wordlist_list = parse.parseMdWordList(marked.parse(mdstr))
